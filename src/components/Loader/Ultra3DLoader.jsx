...
const Ultra3DLoader = () => {
  ...
  useEffect(() => {
    ...
    const torusMaterial = new THREE.ShaderMaterial({
      uniforms: {
        time: { value: 0 },
        colorCycleSpeed: { value: 1.0 }, // Kontrol için yeni uniform
      },
      vertexShader: `
        varying vec2 vUv;
        void main() {
      ...
      fragmentShader: `
        uniform float time;
        uniform float colorCycleSpeed; // Uniform'u aldık
        varying vec2 vUv;

        void main() {
          float speedFactor = 0.5 * colorCycleSpeed; // Hız kontrolü
          float r = 0.5 + 0.5 * sin(time * speedFactor + vUv.x * 10.0);
          float g = 0.5 + 0.5 * sin(time * speedFactor + vUv.y * 10.0 + 2.0);
          float b = 0.5 + 0.5 * sin(time * speedFactor + (vUv.x + vUv.y) * 10.0 + 4.0);
    ...
    return () => {
      ...
      // Dispose icons' materials and textures
      icons.forEach((icon) => {
        icon.traverse((child) => {
          if (child.material) {
  ...
  const createFitnessIcons = () => {
    ...
    for (let i = 0; i < iconCount; i++) {
      ...
      if (i % 3 === 0) {
        ...
        texture.needsUpdate = true; // **Burayı mutlaka ekliyoruz!**
